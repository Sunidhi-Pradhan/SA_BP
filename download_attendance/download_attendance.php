...
.status-leave {
    ...
    display: inline-block;
        ...
        /* ===== STAT CARD VARIANTS ===== */
            ...
            .stat-overtime .stat-label,
            .stat-overtime .stat-value {
                color: #d97706;
            }
...
<body>
   ...
   <script>
    ...
    // Global data store (EMPTY by default – no dummy data)
   ...
   async function previewReport() {
    ...
    try {
        ...
        const response = await fetch(
            `fetch_report.php?employee_id=${employeeId}&from_date=${fromDate}&to_date=${toDate}`
        );
        ...
        document.getElementById('totalRecordsText').textContent =
        `TOTAL RECORDS FOUND: ${result.daysRecorded} out of ${result.totalDays} days`;
        ...
        result.attendanceData.forEach(row => {
            ...
            else if (row.status === 'Present + Overtime') {
                statusClass = 'status-present-overtime';
            }
...
function downloadPDF() {
    ...
    if (!currentReportData || !currentReportData.attendanceData.length) {
        alert("Please preview report first.");
        return;
    }
    ...
    // MCL LOGO BOX
    // ==============================
    doc.setFillColor(16, 121, 96);
    doc.roundedRect(20, 20, 25, 20, 3, 3, "F");

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("MCL", 32.5, 33, { align: "center" });
    ...
    doc.setTextColor(0);
    doc.setFontSize(14);
    doc.text("Mahanadi Coalfields Limited", 55, 27);

    doc.setFontSize(8);
    doc.setTextColor(100);
    doc.text("A Miniratna Subsidiary Company of Coal India Limited", 55, 32);
    doc.text("Jagriti Vihar, Burla, Sambalpur-768020, Odisha", 55, 36);
    ...
    doc.setTextColor(0);
    doc.setFont("helvetica", "bold");
    doc.text("EMPLOYEE ATTENDANCE SHEET", pageWidth / 2, 50, { align: "center" });
    ...
    doc.autoTable({
        startY: y,
        body: [
            ["Employee Name:", currentReportData.employeeName, "Employee ID:", currentReportData.employeeId],
            ["Site Location:", currentReportData.siteLocation, "Period:", currentReportData.period]
        ],
        theme: "grid",
        styles: { fontSize: 8 },
        columnStyles: {
            0: { fontStyle: "bold", fillColor: [230, 240, 240] },
            2: { fontStyle: "bold", fillColor: [230, 240, 240] }
        },
    ...
    doc.autoTable({
        startY: y,
        head: [["Total Days", "Present", "Overtime", "Absent", "Leave"]],
        body: [[
            currentReportData.totalDays,
            currentReportData.present,
            currentReportData.overtime,
            currentReportData.absent,
            currentReportData.leave
        ]],
        theme: "grid",
        headStyles: {
            textColor: 255,
        ...
        didParseCell: function (data) {
            if (data.section === "head") {
                if (data.column.index === 0) data.cell.styles.fillColor = [55, 65, 81];
                if (data.column.index === 1) data.cell.styles.fillColor = [16, 185, 129];
                if (data.column.index === 2) data.cell.styles.fillColor = [59, 130, 246];
                if (data.column.index === 3) data.cell.styles.fillColor = [239, 68, 68];
                if (data.column.index === 4) data.cell.styles.fillColor = [245, 158, 11];
            }
        },
        styles: {
            halign: "center",
    ...
    // ATTENDANCE TABLE
    // ==============================
    const tableRows = currentReportData.attendanceData.map(row => [
        row.sno,
        row.date,
        row.day,
        row.status
    ...
    doc.autoTable({
        ...
        didParseCell: function (data) {
            ...
            if (data.section === "body" && data.column.index === 3) {
                ...
                else if (data.cell.raw === "Overtime") {
                    data.cell.styles.fillColor = [219, 234, 254];
                    data.cell.styles.textColor = [37, 99, 235];
                }
                else if (data.cell.raw === "Present + Overtime") {
...
function downloadExcel() {
    ...
    // COMPANY HEADER
    // =========================
    setCell(row, 0, "Mahanadi Coalfields Limited");
    merge(row, 0, row, 4);
    row++;

    setCell(row, 0, "A Miniratna Subsidiary Company of Coal India Limited");
    merge(row, 0, row, 4);
    row++;

    setCell(row, 0, "Jagruti Vihar, Burla, Sambalpur-768020, Odisha");
    merge(row, 0, row, 4);
    row += 2;
    ...
    // REPORT TITLE
    // =========================
    setCell(row, 0, "EMPLOYEE ATTENDANCE SHEET");
    merge(row, 0, row, 4);
    row += 2;
    ...
    // EMPLOYEE DETAILS
    // =========================
    setCell(row, 0, "Employee Name:");
    setCell(row, 1, d.employeeName);

    setCell(row, 3, "Employee ID:");
    setCell(row, 4, d.employeeId);
    row++;

    setCell(row, 0, "Site Location:");
    setCell(row, 1, d.siteLocation);

    setCell(row, 3, "Period:");
    setCell(row, 4, d.period);
    row += 2;

    // =========================
    // ATTENDANCE SUMMARY
    // =========================
    setCell(row, 0, "ATTENDANCE SUMMARY");
    merge(row, 0, row, 4);
    row++;

    const summaryHeaders = ["Total Days", "Present", "Overtime", "Absent", "Leave"];
    summaryHeaders.forEach((h, i) => setCell(row, i, h));
    row++;
    ...
    summaryValues.forEach((v, i) => setCell(row, i, v));
    row += 2;

    // =========================
    // TABLE HEADER
    // =========================
    const tableHeaders = ["S.NO", "DATE", "DAY", "STATUS"];
    tableHeaders.forEach((h, i) => setCell(row, i, h));
    row++;

    // =========================
    // TABLE DATA
    // =========================
    d.attendanceData.forEach(rec => {
        setCell(row, 0, rec.sno);
        setCell(row, 1, rec.date);
        setCell(row, 2, rec.day);
        setCell(row, 3, rec.status);
        row++;
    });

    // =========================
    // SHEET RANGE + COLUMN WIDTH
    // =========================
