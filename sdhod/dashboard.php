...
// ── Determine selected site & FY ──
$selSite = $_GET['site_code'] ?? 'ALL';
$now     = new DateTime();
...
// ── Active staff count ──
if ($selSite === 'ALL') {
    $activeStaff = (int)$pdo->query("SELECT COUNT(*) FROM employee_master")->fetchColumn();
} else {
...
foreach ($fyMonths as $period) {
    ...
    if ($selSite === 'ALL') {
        ...
        ");
        $st->execute([':yr' => $yr, ':mo' => $mo, ':sc' => $selSite]);
    }
...
<title>Security Attendance and Billing Portal – Dashboard</title>
...
.att-mgmt-card:hover { background: rgba(255,255,255,0.18); }
.att-mgmt-img {
...
.att-mgmt-label { font-size: 0.78rem; color: rgba(255,255,255,0.85); font-weight: 600; letter-spacing: 0.3px; }
...
<div class="layout">
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <ul class="sidebar-nav">
      ...
      <li>
        <a class="nav-link" href="monthlyatt.php">
          <i class="fa-solid fa-calendar-days"></i>
          <span>Monthly Attendance</span>
        </a>
      </li>
      <li>
        <a class="nav-link" href="monthlylpp.php">
          <i class="fa-solid fa-file-lines"></i>
          <span>Monthly LPP</span>
        </a>
      </li>
      <li>
        <a class="nav-link" href="details_monthly_lpp.php">
          <i class="fa-solid fa-list-check"></i>
          <span>Details Monthly LPP</span>
        </a>
      </li>
      <li>
        <a class="nav-link" href="vvstatement.php">
          <i class="fa-solid fa-file-invoice"></i>
          <span>VV Statement</span>
        </a>
      </li>
      <li>
  ...
  <main class="main-content">
    ...
    <div>
      ...
      <div class="page-heading">
        ...
        <div class="filters">
          <!-- Site dropdown – populated from site_master -->
          <div class="filter-wrap" style="display:flex;align-items:center;gap:8px;">
            <span style="font-size:0.85rem;font-weight:600;color:var(--muted);white-space:nowrap;">Site:</span>
            <select class="filter-select" id="siteSelect" onchange="reloadDashboard()">
              <option value="ALL" <?= $selSite === 'ALL' ? 'selected' : '' ?>>All Available Sites</option>
              <?php foreach ($sites as $site): ?>
                <option value="<?= htmlspecialchars($site['SiteCode']) ?>" <?= $selSite === $site['SiteCode'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($site['SiteCode']) ?> – <?= htmlspecialchars($site['SiteName']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
...
<script>
  ...
  function reloadDashboard() {
    ...
    window.location.href = `dashboard.php?site_code=${encodeURIComponent(site)}&fy=${encodeURIComponent(fy)}`;
  }
  ...
  function renderTable() {
    ...
    if (!allRows.length) {
      tbody.innerHTML = '<tr><td colspan="7" class="no-data">No attendance data found for the selected filters.</td></tr>';
