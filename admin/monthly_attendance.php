...
<head>
    <meta charset="UTF-8">
    <title>Monthly Attendance – Security Attendance and Billing Portal</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:"Segoe UI",sans-serif; }
        ...
        /* Ensure sidebar always scrolls its own content on mobile */
        ...
        .att-card {
            ...
            animation:cardPop 0.45s 0.3s ease both;
            opacity:0;
        }
        @keyframes cardPop {
        ...
        .att-card-header .title { font-size:.95rem; font-weight:700; color:var(--text); }
        .att-card-header i { color:#0f766e; font-size:1rem; }
        ...
        .btn-load:hover { background:#0d5f58; transform:translateY(-2px); box-shadow:0 6px 20px rgba(15,118,110,.38); }
        .btn-load:active { transform:translateY(0); }
        .btn-load.loading { opacity:.75; pointer-events:none; }
        ...
        .att-title-text { font-size:1.4rem; font-weight:800; color:var(--text); text-align:center; }
        .att-subtitle   { font-size:.88rem; color:var(--subtext); text-align:center; }
        .att-count      { font-size:1.05rem; font-weight:600; color:#0f766e; }
        ...
        .state-box {
            padding:48px 24px; text-align:center; color:var(--subtext);
        }
        .state-box i { font-size:2.8rem; display:block; margin-bottom:12px; opacity:.3; }
        .state-box p { font-size:.9rem; }
        .spinner {
            display:inline-block; width:32px; height:32px;
            border:3px solid var(--border); border-top-color:#0f766e;
        ...
        @media (max-width:768px) {
            ...
            .att-card { border-radius:12px; }
            .att-card-header { padding:.9rem 1.1rem; }
            .att-card-header .title { font-size:.88rem; }
            .selector-section { padding:20px 18px; gap:16px; }
            .selector-label { font-size:.7rem; }
            .selector-row { flex-direction:column; align-items:stretch; gap:10px; }
            .month-input { min-width:100%; font-size:.88rem; padding:.6rem .9rem; }
            .btn-load { width:100%; justify-content:center; font-size:.88rem; padding:.65rem 1.2rem; }
            .result-inner { padding:28px 18px 32px; gap:18px; }
            .att-title-text { font-size:1.2rem; }
            .att-subtitle { font-size:.83rem; }
            .excel-icon { width:62px; height:62px; }
            .excel-icon::before { font-size:27px; }
            .btn-download { padding:.7rem 1.8rem; font-size:.88rem; width:100%; justify-content:center; }
        }
        ...
        @media (max-width:480px) {
            ...
            .att-card { border-radius:10px; }
            .att-card-header { padding:.8rem 1rem; }
            .selector-section { padding:16px 14px; gap:12px; }
            .selector-label { font-size:.68rem; letter-spacing:.4px; }
            .month-input { font-size:.85rem; padding:.55rem .8rem; border-radius:8px; }
            .btn-load { font-size:.85rem; padding:.6rem 1rem; border-radius:8px; }
            ...
            .att-title-text { font-size:1.05rem; }
            .att-subtitle { font-size:.8rem; }
            .att-count { font-size:.95rem; }
            .btn-download { padding:.65rem 1.4rem; font-size:.83rem; border-radius:10px; }
        }

        /* Very small screens: 360px and below */
        @media (max-width:360px) {
            header h1 { font-size:.85rem; }
            .page-content { padding:10px 8px 20px; }
            .att-card-header .title { font-size:.82rem; }
            .selector-section { padding:14px 12px; }
            .result-inner { padding:18px 12px 22px; }
            .att-title-text { font-size:.98rem; }
            .excel-icon { width:50px; height:50px; }
        ...
        <div class="page-content">
            ...
            <div class="att-card">
                <div class="att-card-header">
                    ...
                    <span class="title">Monthly Attendance Export</span>
                </div>

                <!-- Selector -->
                <div class="selector-section">
                    <div class="selector-label">Select Attendance Month &amp; Year</div>
...
async function loadData() {
    ...
    try {
        const res  = await fetch(`monthly_attendance.php?fetch=1&month=${encodeURIComponent(month)}`);
        ...
        // Build download URL — adjust to your export endpoint
        const downloadUrl = `export_attendance.php?month=${encodeURIComponent(month)}`;
        ...
        resultInner.innerHTML = `
            ...
            <div class="att-title-text">Attendance: ${label}</div>
            <div class="att-subtitle">
                Ready to export <span class="att-count">${Number(count).toLocaleString()} employee records</span>
            </div>
        ...
        loadBtn.innerHTML = '<i class="fa-solid fa-filter"></i> Load Data';
    }
}
</script>
