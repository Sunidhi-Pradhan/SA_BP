...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($userid === "" || $code === "") {
        ...
        // Look up the user
        $stmt = $pdo->prepare("SELECT id, google_secret FROM `user` WHERE id = ?");
        $stmt->execute([$userid]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$user) {
            $error = "Employee ID not found.";
        } elseif (empty($user['google_secret'])) {
            $error = "2FA is not set up for this account. Contact admin.";
        } else {
            // Verify the Google Authenticator code
            $ga = new PHPGangsta_GoogleAuthenticator();
            $valid = $ga->verifyCode($user['google_secret'], $code, 2);
...
<head>
<meta charset="UTF-8">
<title>Forgot Password – Security Attendance and Billing Portal</title>
...
.hint { font-size:.75rem; color:#6b7280; margin-top:.35rem; line-height:1.4; }
...
.btn-verify:hover { filter:brightness(1.07); transform:translateY(-1px); box-shadow:0 6px 18px rgba(15,118,110,0.38); }
...
<div class="page-body">
  <div class="reset-card">
    ...
    <div class="card-header">
      ...
      <p>Step 1: Verify your 2FA code to proceed</p>
    </div>
      ...
      <form method="POST" autocomplete="off">
        ...
        <div class="form-group">
          ...
          <input
            ...
            required
          >
          <p class="hint">Open Google Authenticator and enter your current code.</p>
        </div>
