...
$alreadyApproved = ($currentStep !== 'APM'); // APM already acted if step moved forward
...
if (isset($_POST['approve_report']) && !$alreadyApproved) {
    ...
    if ($workflowRow) {
        /* UPDATE existing row — APM is steps[1] (index 1) */
        $pdo->prepare("
        ...
        /* INSERT new row if somehow not exists */
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .success-banner { width:100%; background:linear-gradient(135deg,#22c55e 0%,#16a34a 60%,#15803d 100%); border-radius:18px; padding:3.5rem 2rem; display:flex; flex-direction:column; align-items:center; gap:1.25rem; box-shadow:0 8px 32px rgba(34,197,94,0.3); position:relative; overflow:hidden; }
        .success-banner::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse at 30% 50%,rgba(255,255,255,0.12) 0%,transparent 60%),radial-gradient(ellipse at 80% 20%,rgba(255,255,255,0.08) 0%,transparent 50%); pointer-events:none; }
    ...
    <div class="user-icon">
        ...
        <div id="attendanceContent" <?= $approvalSuccess ? 'style="display:none"' : '' ?>>
            ...
            <div class="card">
                ...
                <!-- Approval Form (hidden if already approved) -->
                <?php if (!$alreadyApproved): ?>
